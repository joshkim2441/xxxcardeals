...
class ShoppingCart extends DBController
{
    ...
    function updateCart($transport, $insurance, $member_id)
    {
        $query = "UPDATE tbl_cart SET transport = ?, insurance = ? WHERE id = ?";

        $params = array(
            array(
                "param_type" => "i",
                "param_value" => $transport
            ),
            array(
                "param_type" => "i",
                "param_value" => $insurance
            ),
            array(
                "param_type" => "i",
                "param_value" => $member_id
        ...
        $select_services = $this->insertDB($query, $params);
    ...
    function insertOrderItem($order, $product_id, $price, $quantity, $transport, $insurance, $total_cost)
    {
        $query = "INSERT INTO tbl_order_item (order_id, product_id, item_price, quantity, transport, insurance, total_cost) VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    function insertPayment($order, $payment_status, $payment_response)
    {
        ...
        $payment_id = $this->insertDB($query, $params);
        return $payment_id;
    }
